 ...
 *
 * A fork changes six values. They are needed at two different times, on two different
 * machines, with no shared runtime between them: at BUILD time by Astro and by the editor
 * bundle (`jamground.config.mjs`, a plain committed ES module), and at DEPLOY time by Ansible
 ...
 *
 *   RULE A — the two declarations agree, value for value, on all six.
 ...
 *   name, and by name only — its test imports the list from here rather than repeating it,
 *   so nothing else needs the exemption.
 *
...
/** The one file exempt from Rule B: this one, which has to name every forbidden literal. */
...
async function ruleA(root) {
  ...
  try {
    ...
    return { fatal: `${relative(root, configJs)} could not be imported: ${e.message}`,
      hint: 'It must be a plain ES module of literals that resolves in Node with no build step.' };
  }
  ...
  for (const { js, yaml } of SHARED_VALUES) {
    const build = buildTime[js];
    const deploy = deployTime[yaml];
    if (build === undefined) {
    ...
    if (String(build) !== String(deploy)) {
      problems.push({ file: 'jamground.config.mjs', rule: 'config-drift', key: js, build, deploy,
        message: `\`${js}\` is ${JSON.stringify(build)} but \`${yaml}\` in `
          + `infra/ansible/group_vars/all.yml is ${JSON.stringify(deploy)}. The build and the `
...
if (invokedDirectly) {
  ...
  if (report.ok) {
    ...
    console.log(`config — ${SHARED_VALUES.length} shared values agree between `
      + 'jamground.config.mjs and infra/ansible/group_vars/all.yml; '
